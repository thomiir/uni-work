...
void testRepo()
{
	...
	carList* pointer = &testList;
	addCar(pointer, testCar);
	assert (pointer->n == 1);
	assert(strcmp(pointer->cars[0].carLicPlate, "AR-21-GGH") == 0);
	assert(strcmp(pointer->cars[0].carModel, "VITARA") == 0);
	assert(strcmp(pointer->cars[0].carCat, "mini") == 0);
	assert(strcmp(pointer->cars[0].carIsRented, "disponibila") == 0);

	car testCar2 = createCar("AB-04-ORT", "SUPERB", "sport", "inchiriata");
	addCar(pointer, testCar2);
	assert(pointer->n == 2);
	assert(strcmp(pointer->cars[1].carLicPlate, "AB-04-ORT") == 0);
	assert(strcmp(pointer->cars[1].carModel, "SUPERB") == 0);
	assert(strcmp(pointer->cars[1].carCat, "sport") == 0);
	assert(strcmp(pointer->cars[1].carIsRented, "inchiriata") == 0);


	// test pentru findCar()

	assert(findCar(pointer, "AR-21-GGH") == 0);
	assert(findCar(pointer, "AR-21-GGG") == -1);
