...
void testElimina()
{
	...
	// adaugam 3 elemente pentru aceeasi cheie
	md.adauga(1, 12);
	md.adauga(1, 13);
	md.adauga(1, 14);

	// stergem toate cele 3 aparitii ale cheii 1
	assert (md.eliminaAparitiiCheie(3, 1) == 3);

	// acum multidictionarul este vid
	// si adaugam din nou 3 elemente pentru aceeasi cheie
	md.adauga(1, 12);
	md.adauga(1, 13);
	md.adauga(1, 14);

	// incercam sa stergem 5 aparitii ale cheii 1
	// dar gasim doar 3
	assert (md.eliminaAparitiiCheie(5, 1) == 3);
	...
	// si stergem o singura aparitie pentru cheia respectiva
	assert (md.eliminaAparitiiCheie(1, 1) == 1);

	// ne asiguram ca s-a sters o singura aparitie
	assert (md.dim() == 2);

	// stergem un element care nu exista
	assert (md.eliminaAparitiiCheie(1, 2) == 0);

	// incercam sa stergem elemente dintr-un multidictionar vid
	try
	{
		MDO vid = MDO(cresc);
		vid.eliminaAparitiiCheie(1, 1);
		assert (false);
	}
	catch (exception)
	{
