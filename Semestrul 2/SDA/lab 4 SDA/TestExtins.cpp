...
void testMultimeaValorilor()
{
	...
	// incercam sa obtinem multimea valorilor unui dictionar vid
	try
	{ 
		Dictionar dict;
		std::vector<TValoare> val = dict.multimeaValorilor();
		assert (false);
	}
	...
	// verificam ca dimensiunea coincide cu nr elementelor
	assert (dict.dim() == 5);

	// ne asiguram ca sunt adaugate corect in dictionar
	for (int i = 1; i <= 5; i++)
		assert (dict.cauta(i) == i * 100);
}
